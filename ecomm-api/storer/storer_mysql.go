...
func (ms *MySQLStorer) UpdateProduct(ctx context.Context, p *Product) (*Product, error){
	res, err := ms.db.NamedExecContext(ctx, "UPDATE products SET name = :name, image = :image, category = :category, description = :description, rating = :rating, num_reviews = :num_reviews, price = :price, count_in_stock = :count_in_stock WHERE id = :id", p)
	if err != nil {
...
func createOrder(ctx context.Context, tx *sqlx.Tx, o *Order) (*Order, error){
	...
	id, err := res.LastInsertId()
	if err != nil{
		return nil, fmt.Errorf("error geting las insert ID: %w", err)
	}
	o.ID = id
